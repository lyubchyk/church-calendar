class Admin::MasterController < ApplicationController
  ...
  def new
    ...
    %w( controller action resource resource_id back_to selected locale ).each do |param|
  ...
  def edit
    ...
    item_params = params.dup
    %w( action controller model model_id back_to id resource resource_id page locale authenticity_token).each { |p| item_params.delete(p) }
    ...
    #we got to set a different locale for translation
    I18n.locale = params[:locale]
    respond_to do |format|
      format.js { render :template => "admin/resources/edit", :layout => false }
